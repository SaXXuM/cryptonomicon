      ...
      <section v-if="selectedTicker" class="relative">
        ...
        >
          <div
            ...
            ref="graphElement"
            :style="{ height: `${bar}%` }"
            class="bg-purple-800 border w-10 h-24 flex-shrink-0"
          ></div>
        </div>
        <button
...
<script>
import { subscribeToTicker, unsubscribeFromTicker, fetchCoins } from "./api";
...
export default {
  ...
  methods: {
    ...
    updateTicker(tickerName, message) {
      const { price, error } = message;
      console.log(tickerName, message);
      this.tickers
    ...
    add(ticker) {
      if (this.tickers.filter((t) => t.name === ticker).length) {
        this.error = true;
        return;
      }
      const currentTicker = {
    ...
    handleDelete(tickerToRemove) {
      this.tickers = this.tickers.filter(
        (ticker) => ticker.name !== tickerToRemove
      );
